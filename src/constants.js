export const GUEST_NAME = 'Guest';

export const ALERT_TIMER_DURATION = 8000;

// Buttons text
export const LOGOUT_BTN_TEXT = 'Logout';
export const SEARCH_BTN_TEXT = 'Search';
export const SHOW_COURSE_BTN_TEXT = 'Show course';
export const ADD_NEW_COURSE_BTN_TEXT = 'Add new course';
export const CREATE_COURSE_BTN_TEXT = 'Create course';
export const UPDATE_COURSE_BTN_TEXT = 'Update course';
export const CREATE_AUTHOR_BTN_TEXT = 'Add new author';
export const ADD_AUTHOR_BTN_TEXT = 'Add to course';
export const DELETE_AUTHOR_BTN_TEXT = 'Delete from course';
export const REGISTRATION_BTN_TEXT = 'Registration';
export const LOGIN_BTN_TEXT = 'Login';
export const BACK_TO_COURSES_BTN_TEXT = 'Back to courses';
export const BACK_TO_MAIN_PAGE_BTN_TEXT = 'Main Page';
export const DELETE_BTN_ICON = <span className='icon-bin'></span>;
export const EDIT_BTN_ICON = <span className='icon-pencil'></span>;
export const ADD_BTN_ICON = <span className='icon-plus'></span>;
export const CLOSE_BTN_ICON = <span className='icon-cross'></span>;
export const LOGOUT_BTN_ICON = <span className='icon-logout'></span>;
export const SEARCH_BTN_ICON = <span className='icon-search'></span>;
export const LOADING_BTN_SPINNER = (
	<div className='btn-spinner-ellipsis'>
		<div></div>
		<div></div>
		<div></div>
		<div></div>
	</div>
);
export const LOADING_BTN_SPINNER_CIRCLE = (
	<div class='btn-spinner-dual-ring'></div>
);
